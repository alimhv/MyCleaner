...
public class CleanerService extends Service {
    ...
    public static String currentPK="";
...
//                for(counter=0;counter<packages.size();counter++)
                {
                    Log.d(TAG, "doInBackground: "+packages.get(counter).packageName);
                            ...
                            new IPackageStatsObserver.Stub() {
                                ...
                                public void onGetStatsCompleted(PackageStats pStats,
                                    ...
                                    synchronized (apps) {
                                        ...
                                        addPackageKill(apps, pStats, succeeded);
//                                        mCacheSize += addPackage(apps, pStats, succeeded);
          ...
          //  }
            ...
            try {
                ...
                ApplicationInfo info = packageManager.getApplicationInfo(pStats.packageName,
                        PackageManager.GET_META_DATA);
                //CleanerFragment.mpknameText.setText(pStats.packageName);
                currentPK=pStats.packageName;
                appicon=packageManager.getApplicationIcon(pStats.packageName);
        ...
        private void addPackageKill(List<AppsListItem> apps, PackageStats pStats, boolean succeeded) {
            long cacheSize = 1;
//
//            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
//                cacheSize += pStats.cacheSize;
...
//            }
            ...
            try {
                ...
                ApplicationInfo info = packageManager.getApplicationInfo(pStats.packageName,
                    PackageManager.GET_META_DATA);
                if((info.flags & ApplicationInfo.FLAG_SYSTEM)!=1&&info.packageName!=getApplicationContext().getPackageName()){
                    Log.d(TAG, "addPackageKill: "+((info.flags & ApplicationInfo.FLAG_SYSTEM))+":"+counter+":"+info.packageName);
                    //CleanerFragment.mpknameText.setText(pStats.packageName);
                currentPK=pStats.packageName;
                appicon=packageManager.getApplicationIcon(pStats.packageName);
                apps.add(new AppsListItem(pStats.packageName,
                        ...
                        if (externalDataDirectory.isDirectory()) {
                            ...
                            for (File file : files) {
                                if (!deleteDirectory(new File(String.format(externalCachePath,
                                    ...
                                    Log.d(TAG, "doInBackground: "+ file.getName());
                                }
                            }
                        } else {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (action.equals(ACTION_CLEAN_AND_EXIT)) {
            if (!canCleanExternalCache(this)) {
                ...
                Toast.makeText(this, ("R.string.toast_could_not_clean_reason"+"R.string.rationale_title"), Toast.LENGTH_LONG).show();
